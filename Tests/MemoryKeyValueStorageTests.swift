		...
		describe("MemoryKeyValueStorage") {
			...
			context("initialization") {
				...
				context("with no parameters") {
					...
					it("should have the standard Memory") {
				...
				context("with a nil parameter") {
					...
					beforeEach {
						storage = MemoryKeyValueStorage(storage: nil)
						storage?.clear()
					}
				...
				context("with a valid parameter") {
					...
					beforeEach {
						memory = [:]
						storage = MemoryKeyValueStorage(storage: memory)
						storage?.clear()
					}
					...
					it("should have the standard Memory") {
						expect(storage?.storage).to(beIdenticalTo(memory))
					}
				}
			}
