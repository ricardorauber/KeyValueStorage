...
class UserDefaultsKeyValueStorageTests: QuickSpec {
	override func spec() {
		
		var storage: UserDefaultsKeyValueStorage?
		
		describe("UserDefaultsKeyValueStorage") {
			...
			context("initialization") {
				...
				context("with no parameters") {
					...
					beforeEach {
						storage = UserDefaultsKeyValueStorage()
						storage?.clear()
					}
					...
					it("should have the standard UserDefaults") {
						expect(storage?.storage).to(beIdenticalTo(UserDefaults.standard))
					}
				}
				...
				context("with a nil parameter") {
					...
					beforeEach {
						storage = UserDefaultsKeyValueStorage(storage: nil)
						storage?.clear()
					}
				...
				context("with the standard UserDefaults as a parameter") {
					
					beforeEach {
						storage = UserDefaultsKeyValueStorage(storage: UserDefaults.standard)
						storage?.clear()
					}
				...
				context("with a valid parameter") {
					...
					beforeEach {
						...
						storage = UserDefaultsKeyValueStorage(storage: defaults)
						storage?.clear()
					}
					...
					it("should have the standard UserDefaults") {
						expect(storage?.storage).to(beIdenticalTo(defaults))
					}
				}
			}
